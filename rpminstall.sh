...
finalize_yum_repo ()
{
  ...
  yum install -y pygpgme --disablerepo='scalr_scalr-server-ee'
  ...
  if [ "$?" != "0" ]; then
    ...
    sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/scalr_scalr-server-ee.repo
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='scalr_scalr-server-ee'
  ...
  echo "Generating yum cache for scalr_scalr-server-ee..."
  yum -q makecache -y --disablerepo='*' --enablerepo='scalr_scalr-server-ee'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh scalr_scalr-server-ee
}
...
main ()
{
  ...
  yum_repo_config_url="https://9a2b3339b0b29641ea857f8e1b1051faa540c9544db4f47c:@packagecloud.io/install/repositories/scalr/scalr-server-ee/config_file.repo?os=${os}&dist=${dist}&name=${unique_id}&source=script"
  echo "Found unique id: ${unique_id}"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/scalr_scalr-server-ee.repo
  else
    yum_repo_path=/etc/yum.repos.d/scalr_scalr-server-ee.repo
  fi
